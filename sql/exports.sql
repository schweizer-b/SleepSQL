-- Export method: DBeaver 🦫✅

-- Set: CSV and Encoding: UTF-8

-- A) Main table: bi_nights.csv = one row per recording/night
    -- Includes: sleep metrics (from v_sleep_summary) + demographics + notes + derived QC (pass_qc, qc_reason)
...
-- B) bi_stage_minutes.csv = one row per night × stage
    -- for stacked bar 'sleep architecture'
...
-- C) bi_fragmentation.csv = one row per night
    -- Includes transitions + awakenings (fragmentation) normalised per hour
